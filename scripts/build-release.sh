...
if [ "$#" -ne 1 ]; then
    echo "Usage: $0 /absolute/path/ModuleOpenLdapSync.1.1.1.zip" >&2
    exit 2
fi
...
temporary_dir=$(mktemp -d "$output_dir/.ModuleOpenLdapSync.1.1.1.XXXXXX")
temporary_archive="$temporary_dir/ModuleOpenLdapSync.1.1.1.zip"
